...
def address_sanitizer(args, program_args):
    ...
    run(cmd, env_vars={'ASAN_OPTIONS': f'exitcode=2 verbosity=1 log_path={args.tsan_log_path}'}, debug=True)

def thread_sanitizer(args, program_args):
    cmd = [ args.program_path ]
    cmd.extend(program_args)
    run(cmd, env_vars={'TSAN_OPTIONS': f'exitcode=2 verbosity=1 log_path={args.asan_log_path}'}, debug=True)
